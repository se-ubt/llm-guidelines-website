...
\comment{In the text we underline the importance of prompt engineering? Do you think we can provide some extra guidance by providing a practical guide on prompt engineering in this specific case? What do the examples give us in terms of prompt engineering examples? I didn't know what to suggest here so I asked Claude (see what I've done?!? I've disclosed it :-) to 'provide me with prompt engineering advices for researchers who want to use LLMs for simulating subjects in empirical experiments'; this is what it gave me -- take it with grain of salt: "When designing prompts for subject simulation, researchers should: a. Include specific background context (e.g., 'You are a Java developer with 5 years of experience working on microservices'); b. Frame questions as they would appear in actual research instruments (i.e., to mirror how questions would be presented to human participants); c. Include consistent guidelines on response format and length (e.g., 'Please respond with 3-5 sentences explaining your reasoning. Include at least one concrete example from your experience.'); Not sure all them make sense, but maybe they do for you. Feel free to drop them altogether though. }
...
\comment{Another practical guidance that we can provide is about reporting limitations of using LLMs as subjects. How about: "When reporting results, researchers should address in the  'Limitations' section: (1) How LLM subject responses might differ from human responses; (2) Which findings might be artifacts of LLM training rather than representative of human behavior (note to clarify, we should say that researcher really really need to critically examine whether patterns in their results genuinely reflect human behavior or are rather artifacts of how LLMs process information based on their training data and architecture); (3) If this is a case where LLM responses are least likely to match human responses (e.g., novel technologies (to clarify, data unseend by LLMs), niche tasks)."  }
